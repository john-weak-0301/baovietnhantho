...
async function putBlock (options, matches) {
  let id = matches[1]
  let response = await axios.put(`/dashboard/gutenberg/blocks/${id}`, options.data)
  return response.data
}
...
function matchPath (options) {
  ...
  Object.keys(requests).forEach((key) => {
    ...
    if ((options.method === request.method || (!options.method && request.method === 'GET')) &&
      matches && matches.length > 0) {
      promise = request.run(options, matches).catch(() => Notices.error('Could not complete request.'))
    }
  })
