...
abstract class Trend extends RangedMetric
{
    ...
    protected function aggregate($request, $model, $unit, $function, $column, $dateColumn = null)
    {
        ...
        $possibleDateResults = $this->getAllPossibleDateResults(
            ...
            $unit,
            $timezone,
            $request->input('twelveHourTime') === 'true'
        );
        ...
        $results = array_merge($possibleDateResults, $results->mapWithKeys(function ($result) use ($request, $unit) {
            return [
                $this->formatAggregateResultDate(
                    ...
                    $unit,
                    $request->input('twelveHourTime') === 'true'
                ) => round($result->aggregate),
            ];
        })->all());

        if (count($results) > $request->input('range')) {
            array_shift($results);
        }
    ...
    protected function getAggregateStartingDate($request, $unit)
    {
        ...
        switch ($unit) {
            ...
            case 'hour':
                return with($now->subHours($range - 1), function (Carbon $now) {
                    return $now->setTimeFromTimeString($now->hour.':00');
                });

            case 'minute':
                return with($now->subMinutes($range - 1), function (Carbon $now) {
    ...
    protected function formatAggregateResultDate($result, $unit, $twelveHourTime)
    {
        switch ($unit) {
            ...
            case 'day':
                return with(Carbon::createFromFormat('Y-m-d', $result), function (Carbon $date) {
    ...
    protected function formatAggregateMonthDate($result)
    {
        ...
        return with(Carbon::create((int) $year, (int) $month), function (Carbon $date) {
    ...
    protected function getAllPossibleDateResults(
        ...
        $possibleDateResults[$this->formatPossibleAggregateResultDate($nextDate, $unit, $twelveHourTime)] = 0;
