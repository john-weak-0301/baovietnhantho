...
class StorageController extends Controller
{
    ...
    {
        $this->authorize(MediaPermission::BROWSE_MEDIA);

        return view('core::storage.index');
    }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(),
                        $allowedMimeTypes))) {
                throw new Exception(__('voyager::generic.mimetype_not_allowed'));
            }

            if (!$request->has('filename') || $request->get('filename') == 'null') {
                while (Storage::disk($this->filesystem)->exists(Str::finish($request->upload_path,
                        '/').$name.'.'.$extension, $this->filesystem)) {
                ...
                if ($request->file->getClientOriginalExtension() == 'gif') {
                    ...
                    if (property_exists($details, 'thumbnails') && is_array($details->thumbnails)) {
                        foreach ($details->thumbnails as $thumbnail_data) {
                            ...
                            $thumbnail->save($realPath.$request->upload_path.$name.'-'.($thumbnail_data->name ?? 'thumbnail').'.'.$extension,
                                ($details->quality ?? 90));
                        }
                    }
                    // Add watermark to image
                    if (property_exists($details, 'watermark') && property_exists($details->watermark,
                            'source')) {
    ...
    protected function addWatermarkToImage($image, $options)
    {
